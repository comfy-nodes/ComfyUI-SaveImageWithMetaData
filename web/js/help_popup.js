// https://github.com/kijai/ComfyUI-KJNodes/blob/main/web/js/help_popup.js
...
export const loadScript = (
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (existingScript) {
        resolve({ status: true, message: '[SaveImageWithMetaData] Script already loaded' })
        return
      }
      ...
      scriptEle.addEventListener('error', (ev) => {
        reject({
          status: false,
          message: `[SaveImageWithMetaData] Failed to load the script ${FILE_URL}`,
        })
      })
...
// Initialize script loading with fallback mechanism
const initializeScripts = async () => {
  // Try to load KJ's scripts first
  const kjMarkedLoaded = await loadScript('kjweb_async/marked.min.js').then(() => true).catch(() => false)
  const kjPurifyLoaded = await loadScript('kjweb_async/purify.min.js').then(() => true).catch(() => false)
  
  if (kjMarkedLoaded && kjPurifyLoaded) {
    console.log('[SaveImageWithMetaData] Help Popup: using scripts from KJNodes')
  } else {
    // Fallback to SaveImageWithMetaData scripts directory
    await loadScript('/web/js/async/marked.min.js').catch((e) => console.log(e))
    await loadScript('/web/js/async/purify.min.js').catch((e) => console.log(e))
    console.log('[SaveImageWithMetaData] Help Popup: using scripts from "web/js/async"')
  }
}

// Initialize scripts on module load
initializeScripts().catch((e) => console.error('[SaveImageWithMetaData] Failed to initialize scripts:', e))

const nodeNames = ["SaveImageWithMetaData"];
app.registerExtension({
	name: "SaveImageWithMetaData.HelpPopup",
	async beforeRegisterNodeDef(nodeType, nodeData) {
  
  // if (app.ui.settings.getSettingValue("SaveImageWithMetaData.helpPopup") === false) {
  //   return;
  //   }
		try {
			nodeNames.forEach(nodeName => {
        if (nodeData?.name?.startsWith(nodeName)) {
            addDocumentation(nodeData, nodeType);
        }
        else return
    });
		} catch (error) {
			console.error("Error in registering SaveImageWithMetaData.HelpPopup", error);
		}
	},
});

const create_documentation_stylesheet = () => {
    const tag = 'saveimagewithmetadata-documentation-stylesheet'
    ...
    if (!styleTag) {
      ...
      styleTag.id = tag
      styleTag.innerHTML = `
      .saveimagewithmetadata-documentation-popup {
  ...
  export const addDocumentation = (
    ...
    nodeType.prototype.onDrawForeground = function (ctx) {
      ...
      if (this.show_doc && docElement === null) {
        ...
        docElement.classList.add('saveimagewithmetadata-documentation-popup')
